...
class Home extends Component {
  ...
  getCountryDetails = async () => {
    ...
    console.log(data)
    if (response.ok) {
      const newData = statesList.map(item => {
  ...
  searchInputChange = input => {
    const newSearchInputList = statesList.filter(item =>
      item.state_name.toLowerCase().includes(input.target.value.toLowerCase()),
    )
    this.setState({
      searchList: newSearchInputList,
      searchInput: input.target.value,
    })
  }

  renderSuccessView = () => {
    const {countriesData, searchInput, searchList} = this.state
    const confirmedArray = countriesData.map(eachValue => eachValue.confirmed)
    const activeArray = countriesData.map(eachValue => eachValue.active)
    const recoveredArray = countriesData.map(eachValue => eachValue.recovered)
    const deceasedArray = countriesData.map(eachValue => eachValue.deceased)
